...
find_graphics_card(addr_t frameBuffer, addr_t& base, size_t& size)
{
		...
		for (uint32 i = 0; i < 6; i++) {
			if (info.u.h0.base_registers[i] <= frameBuffer
				&& info.u.h0.base_registers[i] + info.u.h0.base_register_sizes[i]
					> frameBuffer) {
				// found it!
				dprintf(DEVICE_NAME ": find_graphics_card() found our device!\n");
	...
	put_module(B_PCI_MODULE_NAME);
	dprintf(DEVICE_NAME ": find_graphics_card() couldn't find matching PCI device\n");
...
vesa_set_display_mode(vesa_info& info, uint32 mode)
{
	...
	if (status != B_OK) {
		dprintf(DEVICE_NAME": framebuffer_set_display_mode(): cannot get mode info\n");
		goto out;
	}
	...
	if (status != B_OK) {
		dprintf(DEVICE_NAME": framebuffer_set_display_mode(): cannot set mode\n");
		goto out;
	}
