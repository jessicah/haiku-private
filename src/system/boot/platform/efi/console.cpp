 ...
 * reserved.
 * Copyright 2018 Jessica Hamilton, jessica.l.hamilton@gmail.com.
...
console_clear_screen(void)
{
	...
	int32 background = ansiToRGB(0);

	for (int i = 0; i < gKernelArgs.frame_buffer.width
			* gKernelArgs.frame_buffer.height; i++) {
		base[i * mul] = (background >> 16) & 0xFF;
		base[i * mul + 1] = (background >> 8) & 0xFF;
		base[i * mul + 2] = background & 0xFF;
	}
}
